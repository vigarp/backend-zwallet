...
const showHistory = ({idUser, orderQuery, limitQuery, offsetQuery}) => {
    return new Promise((resolve, reject) => {
        connection.query(`SELECT transactions.invoice, transactions.id_sender,transactions.id_receiver, IF(transactions.id_sender = ${idUser}, u1.username, u2.username) as username,  IF(transactions.id_sender = ${idUser}, u1.picture, u2.picture) as picture, transactions.type, IF(transactions.type = 'Topup', 'Topup', IF(transactions.id_sender = ${idUser}, 'Transfer Out', 'Transfer In')) as type_detail, transactions.amount, transactions.notes, transactions.created_at FROM transactions LEFT JOIN users u1 ON (u1.id = transactions.id_receiver) LEFT JOIN users u2 ON (u2.id = transactions.id_sender) WHERE (id_sender=${idUser} OR id_receiver=${idUser}) ORDER BY created_at ${orderQuery} LIMIT ${limitQuery} OFFSET ${offsetQuery};`, (error, result) => {
            if (!error) {
                resolve(result)
            } else {
                console.log(error);
...
// create model for count transaction history
const countTransaction = () => {
    return new Promise((resolve, reject) => {
        connection.query('SELECT COUNT(*) AS total_transactions FROM transactions', (error, result) => {
            if (!error) {
                resolve(result)
            } else {
                console.log(error);
